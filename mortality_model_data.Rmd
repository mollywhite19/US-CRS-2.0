...
table_data = df_final %>% filter(!is.na(status)) %>%
  ...
  ## NOTE: we should run with the full dataset (or at least full training set) for the paper
...
# Create duplicate observations for each landmark - START HERE
```{r}
...
combined_df %>% select(PX_ID, interval_start, interval_stop, time_until_death_after_delisting, outcome) %>% filter(outcome == 1)

combined_df
```
```{r}
combined_df %>% select(PX_ID, interval_start, interval_stop, time_until_death_after_delisting, outcome) %>% filter(PX_ID == 1254863)
```
...
mortality_model_no_weights1 = glm(formula = as.formula(
  outcome ~ 
    albumin_at_start + log(bilirubin_at_start + 1) + eGFR_at_start + log(BNP_at_start + 1):as.character(BNP_NT_Pro_at_start) +  sodium_at_start + durable_LVAD_at_start + short_mcs_ever_at_start + cpo_at_start + api_at_start + papi_at_start), data = subset(combined_df,landmark==1), family = binomial)
...
mortality_model_tweights = glm(formula = as.formula(
  outcome ~ 
    albumin_at_start + log(bilirubin_at_start + 1) + eGFR_at_start + log(BNP_at_start + 1):as.character(BNP_NT_Pro_at_start) + sodium_at_start + durable_LVAD_at_start + short_mcs_ever_at_start + cpo_at_start + api_at_start + papi_at_start), data = combined_df, weights = tweight, family = binomial)
...
```
```{r}
df_final %>% filter(is.na(durable_lvad_implant_dt) & durable_LVAD == 1)
df_final %>% filter(status_criteria == "CriteriaLvadSupport") %>% select(current_date, durable_lvad_implant_dt, durable_LVAD)
