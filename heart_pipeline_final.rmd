...
This code pulls in all of the SRTR SAFs needed to create a time-varying dataset of adult heart candidates who were alive and waiting during the calendar period defined by `filter_date_start` and `filter_date_end`. The result is a time-varying dataset that includes rows prior to this calendar period.
...
if (RData_file_loaded == FALSE) {
  ...
  cand_thor = read_sas(here::here(curr_data_release, "cand.sas7bdat"), NULL) %>%  
    zap_formats() %>% zap_labels()
  ...
  stathist_thor = 
    read_sas(here::here(curr_data_release, "stathist.sas7bdat"), NULL) %>%  
    zap_formats() %>% zap_labels()
  ...
  curr_data_release = file.path(curr_data_release,"ThoracicRegistration")
...
# right_join to filter out erroneous forms from data link, as above
just_form_hr_data_link = just_form_hr_data_link %>%
  select(-ChangeDate) %>% 
  right_join(risk_strat_data_hr %>% 
              select(ChangeDt, JustId, PX_ID), by = "JustId")

# right_join to filter out erroneous forms from just_form_hr, as above
...
lvads = thor_support_device %>% 
  
  right_join(just_form_hr %>% select(ThorSupportDevicePrimary, JustId),
            by = c("DeviceId" = "ThorSupportDevicePrimary")) %>%
  left_join(just_form_hr_stat4, by = c("JustId")) %>%

  # collapse all the brands
  ## 239 is a PercuBrandId
  mutate(BrandId = coalesce(VadBrandId,TahBrandId,PercuBrandId)) %>%
  
  # restrict to left ventricular support or other/specify
  ## 2=right, 3=single
  filter(VentricularSupportId %in% 1 |
           (is.na(VentricularSupportId) & BrandId %in% 999)) %>%

  # filter to desired LVADs
  filter(BrandId %in% c('202', '205', '206', '207', '208', '209', '210', 
                        '212', '213', '214', '223', '224', '233', '236', 
                        '239', '240', '312', '313', '314', '315', '316', '319', '322', 
                        '327', '330', '333', '334') | (BrandId == 999 & CriteriaLvadSupport == 1) | (BrandId == 999 & grepl(paste(lvad_keywords, collapse = "|"), OtherSpecify, ignore.case = TRUE)))
# table(lvads$BrandId,useNA="ifany)
# 
#   205   209   223   224   233   236   239   999 
#  7255    25    79 11863    20 25897  1224    85 

# get earliest implant date per PX_ID
lvads_earliest = lvads %>% group_by(PX_ID) %>% mutate(
  durable_lvad_implant_dt = min(ImplantDt,na.rm=T)
) %>% select(PX_ID, durable_lvad_implant_dt) %>% distinct()
```
...
cand_list = cand_thor %>% 
  ...
  # Filter to candidates listed during prevalent window
  filter(is.na(CAN_REM_DT) | CAN_REM_DT  >= filter_date_start) %>%
...
if (!grace) {
  ...
  rm(risk_strat_data_hr, stathist_thor, just_form_hr,
     stat_just_hr1a, stat_just_hr1b, just_form_hr_stat1, just_form_hr_stat2,
     just_form_hr_stat3, just_form_hr_stat4)
}
...
# create unique_event and unique_date variables
cand_list = cand_list %>%
...
# Pivot tx data by date, then join with full_list
...
# pivot by date, as above
tx_list = tx_list %>% 
  rename(TFL_LAST_FU_DT = TFL_LAFUDATE) %>%
  pivot_longer(cols = ends_with("DT"),
               names_to = "event", 
               values_to = "date", 
               values_drop_na = TRUE) %>%
...
# select certain variables for risk_strat
risk_strat = risk_strat %>%   
...
# when there are two distinct justification forms that have an equivalent date for a specific variable, this code updates the date of the latter form to be equivalent to the ChangeDate. So for example, if a patient has two heart rate measurements on the same date for different JustIds, the JustId that was created later updates the date of the heart rate measurement on that justification form to be equivalent to the ChangeDate for that form.
...
risk_strat = risk_strat %>% 
  left_join(merged_JustId %>% distinct, 
...
# pivot wider
risk_strat = risk_strat %>% 
  # remove this variable so that results are unique by px_id, justid, unique_date, unique_event
  select(-date_changed) %>%
...
# create a column for which criteria was met at the reporting time
risk_strat = risk_strat %>%
...
# note: can have multiple rows for same date/event if different justification forms
## AND these rows have different data
## solution: share the data across both rows, then take the earlier
risk_strat %>% group_by(PX_ID,unique_event,unique_date) %>% filter(n()>1)
risk_strat = risk_strat %>%
  group_by(PX_ID, unique_event, unique_date) %>%
  fill(status_criteria:RequestedCandStatCd, .direction = "downup") %>%
  slice(1) %>% 
  ungroup()

# join just form data with cand, stathist, and tx data
full_list = full_list %>%
...
if (!grace) {
  ...
  save(df,file="cont_time_series_2403.Rdata")
...
# remove people who died before filter_date_start
df = df %>% filter(is.na(death_date_max) | death_date_max >= filter_date_start)
```

# SRTR internal only: Look at ESRD and restricted deaths
```{r}
if (grace) {
  df = left_join(df,cand_thor %>% select(PX_ID,PERS_ESRD_DEATH_DT,PERS_RESTRICT_DEATH_DT,PERS_RESTRICT_DEATH_VRFY),by="PX_ID")
  
  # look for people missing death date from other sources who have an ESRD or restricted death date within 6 weeks of removal (not for deterioration)
  df %>% distinct(PX_ID) %>% nrow # 35138 candidates
  df %>% 
    filter(is.na(death_date_max),CAN_REM_CD!=13,
           !is.na(PERS_ESRD_DEATH_DT) | !is.na(PERS_RESTRICT_DEATH_DT)) %>%
    distinct(PX_ID) %>% nrow # 66 candidates have an ESRD or restricted death date, with no other death date
  df %>% 
    filter(is.na(death_date_max),CAN_REM_CD!=13,
           !is.na(PERS_ESRD_DEATH_DT) | !is.na(PERS_RESTRICT_DEATH_DT)) %>%
    distinct(PX_ID,PERS_ESRD_DEATH_DT,PERS_RESTRICT_DEATH_DT) %>% 
    count(is.na(PERS_ESRD_DEATH_DT),is.na(PERS_RESTRICT_DEATH_DT)) # all restricted
  df %>% 
    filter(is.na(death_date_max),CAN_REM_CD!=13,
           !is.na(PERS_ESRD_DEATH_DT) | !is.na(PERS_RESTRICT_DEATH_DT)) %>%
    filter(pmin(PERS_ESRD_DEATH_DT,PERS_RESTRICT_DEATH_DT,na.rm=T)-removal_date <= weeks(6) | is.na(removal_date)) %>%
    distinct(PX_ID) %>% nrow # only 6 were within 6 weeks of removal
  
  # remove the new variables; no need
  df = df %>%
    select(-PERS_ESRD_DEATH_DT,-PERS_RESTRICT_DEATH_DT,-PERS_RESTRICT_DEATH_VRFY)
}
```
...
# note: these delistings are always the last row
df %>% group_by(PX_ID) %>% filter(any(grepl("CAN_REM_DT", unique_event) & transplant==0 & t_start==t_stop & row_number()!=n())) %>% nrow()

# what's going on with rows after delisting when there was no death?
## conclusion: just data byproducts, eg ChangeDt after removal
df %>% group_by(PX_ID) %>% filter(any(grepl("CAN_REM_DT", unique_event) & transplant==0 & t_start!=t_stop & outcome!=1),t_start>=removal_time) %>%
...
tmp2 = df %>% group_by(PX_ID) %>% filter(transplant <= 1 &
                      ((t_start==t_stop & transplant==1) |
                         (t_start==t_stop & grepl("CAN_REM_DT",unique_event) & transplant==0 & outcome==0) |
                         (t_start!=t_stop) |
                         n() == 1))
dim(table(tmp2$PX_ID))
table(tmp2$transplant)
table(tmp2$outcome) # too many
tmp2 %>% filter(transplant==0,outcome==0,grepl("CAN_REM_DT",unique_event)) %>% nrow()
...
# Join earliest LVAD implant dates to df
```{r}
df = df %>% left_join(lvads_earliest, by = "PX_ID")
...
df = df %>% ungroup() %>% mutate(
  durable_LVAD = case_when(
    durable_lvad_implant_dt <= unique_date |
...
# Add in BNP test type and fix sodium (plausible values)
