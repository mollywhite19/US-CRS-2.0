...
thor_support_device = thor_support_device %>% mutate(
  ChangeDt = as.Date(ChangeDate)
)
...
lvads = thor_support_device %>% left_join(just_form_hr_stat4, by = c("PX_ID", "ChangeDt")) %>% filter(VadBrandId %in% c('202', '205', '206', '207', '208', '209', '210', 
...
lvads = lvads %>% group_by(PX_ID) %>% mutate(
  durable_lvad_implant_dt = first(ImplantDt)
) %>% select(PX_ID, durable_lvad_implant_dt) %>% distinct()

# just_form_hr_stat4 %>% filter(PX_ID == 1131758)
# lvads %>% filter(PX_ID == 1131758)
# 
# df %>% filter(PX_ID == 1131758) %>% select(unique_date, status, status_criteria, CriteriaLvadSupport)

px_ids = (just_form_hr %>% filter(ChangeDt < "2018-10-18"))$PX_ID
px_ids_1a = (stat_just_hr1a %>% filter(PX_ID %in% px_ids))$PX_ID

just_form_hr_stat4 %>% filter(PX_ID == 729036)
cand_thor %>% filter(PX_ID == 1292886)
```
...
cand_list = cand_thor %>% 
  ...
  filter(CAN_LAST_ACT_STAT_DT  >= filter_date_start |     CAN_LAST_INACT_STAT_DT  >= filter_date_start) %>%
...
stathist %>% filter(PX_ID == 729036)
just_form %>% filter(PX_ID == 729036)

cand_list %>% filter(PX_ID == 1343464)
```
...
full_list %>% filter(PX_ID == 1215709)
...
risk_strat %>% select(ends_with("Dt"))
...
risk_strat %>% filter(PX_ID == 729036)
...
```
```{r}
risk_strat = risk_strat %>% mutate(
  across(starts_with("Criteria"), ~ as.numeric(.))) %>%
mutate(
  across(starts_with("Criteria"), ~ 
    ifelse(unique_event == "ChangeDt" & !is.na(.) & . == 1, 1, 0))) 
...
```{r}
df = df %>% left_join(lvads, by = "PX_ID")
...
```
```{r}
df %>% filter(durable_lvad_implant_dt > unique_date & CriteriaLvadSupport == 1) %>% select(durable_lvad_implant_dt, unique_date, CriteriaLvadSupport, status)
```
```{r}
full_list %>% filter(PX_ID == 729036) %>% select(JustId, unique_date, unique_event, CriteriaLvadSupport)

just_stat4 %>% filter(PX_ID == 729036)
stathist %>% filter(PX_ID == 729036)
risk_strat %>% filter(PX_ID == 729036)
```
...
load("heart_post_policy_full.RData")
